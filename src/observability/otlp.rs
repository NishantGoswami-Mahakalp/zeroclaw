...
impl FromStr for OtlpProtocol {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            "protobuf" | "proto" | "binary" => Ok(Self::Protobuf),
            "json" => Ok(Self::Json),
            _ => Err(format!("Unknown protocol: {}. Use 'protobuf' or 'json'", s)),
        }
    }
}
...
impl From<OtlpSampler> for Sampler {
    fn from(sampler: OtlpSampler) -> Self {
        match sampler {
            OtlpSampler::AlwaysOn => Sampler::AlwaysOn,
            OtlpSampler::AlwaysOff => Sampler::AlwaysOff,
            OtlpSampler::ParentBasedAlwaysOn => Sampler::ParentBased(Box::new(Sampler::AlwaysOn)),
            OtlpSampler::ParentBasedAlwaysOff => Sampler::ParentBased(Box::new(Sampler::AlwaysOff)),
        }
    }
}
    ...
    pub fn with_config(
        ...
        let span_exporter = opentelemetry_otlp::SpanExporter::builder()
            ...
            .with_protocol(match protocol {
                OtlpProtocol::Protobuf => opentelemetry_otlp::Protocol::HttpBinary,
                OtlpProtocol::Json => opentelemetry_otlp::Protocol::HttpJson,
            })
            .build()
            .map_err(|e| format!("Failed to create OTLP span exporter: {e}"))?;

        let sampler_clone: Sampler = sampler.into();
        let tracer_provider = SdkTracerProvider::builder()
            .with_sampler(sampler_clone)
        ...
        match event {
            ObserverEvent::AgentStart { provider, model } => {
                ...
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("agent.start")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
                            KeyValue::new("event", "agent_start"),
                            KeyValue::new("provider", provider.clone()),
                            KeyValue::new("model", model.clone()),
                        ]),
                );
            ...
            ObserverEvent::ChannelMessage { channel, direction } => {
                ...
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("channel.message")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
                            KeyValue::new("channel", channel.clone()),
                            KeyValue::new("direction", direction.clone()),
            ...
            ObserverEvent::TurnComplete => {
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("turn.complete")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![KeyValue::new("event", "turn_complete")]),
                );
                span.set_status(Status::Ok);
                span.end();
            }
...
mod tests {
    ...
    fn test_observer() -> OtlpObserver {
        OtlpObserver::new(Some("http://127.0.0.1:19999"), Some("zeroclaw-test"))
            .expect("observer creation should not fail with valid endpoint format")
    }

    #[test]
    fn otlp_observer_name() {
        let obs = test_observer();
        assert_eq!(obs.name(), "otlp");
    }

    #[test]
    fn otlp_observer_with_protocol() {
        let obs = OtlpObserver::with_config(
            Some("http://127.0.0.1:19999"),
            Some("zeroclaw-test"),
            OtlpProtocol::Json,
            OtlpSampler::AlwaysOn,
        )
    ...
    #[test]
    fn otlp_observer_with_sampler_always_off() {
        let obs = OtlpObserver::with_config(
            Some("http://127.0.0.1:19999"),
            Some("zeroclaw-test"),
            OtlpProtocol::Protobuf,
            OtlpSampler::AlwaysOff,
        )
        .expect("observer creation should not fail");
        assert_eq!(obs.name(), "otlp");
    }
    ...
    fn otlp_protocol_from_str() {
        assert_eq!(
            OtlpProtocol::from_str("protobuf").unwrap(),
            OtlpProtocol::Protobuf
        );
        assert_eq!(
            OtlpProtocol::from_str("proto").unwrap(),
            OtlpProtocol::Protobuf
        );
        assert_eq!(
            OtlpProtocol::from_str("binary").unwrap(),
            OtlpProtocol::Protobuf
        );
        assert_eq!(OtlpProtocol::from_str("json").unwrap(), OtlpProtocol::Json);
        assert_eq!(OtlpProtocol::from_str("JSON").unwrap(), OtlpProtocol::Json);
        assert!(OtlpProtocol::from_str("unknown").is_err());
    }
    ...
    #[test]
    fn otlp_records_error_event_without_panic() {
        let obs = test_observer();
        obs.record_event(&ObserverEvent::Error {
            component: "provider".into(),
    ...
    #[test]
    fn otlp_records_llm_failure_without_panic() {
        let obs = test_observer();
        obs.record_event(&ObserverEvent::LlmResponse {
            provider: "openrouter".into(),
    ...
    #[test]
    fn otlp_flush_idempotent_with_unreachable_endpoint() {
        let obs = test_observer();
        obs.flush();
        obs.flush();
        obs.flush();
    ...
    #[test]
    fn otlp_observer_creation_with_valid_endpoint_succeeds() {
        let result = OtlpObserver::new(Some("http://127.0.0.1:12345"), Some("zeroclaw-test"));
        assert!(
            result.is_ok(),
